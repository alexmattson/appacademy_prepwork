...
class String
  def caesar(shift)
    arr = self.split("").map do |letter|
      num = letter.ord
      upperCase = (num < 90)
      if (num + shift > 122 && !upperCase) || (num + shift > 90 && upperCase)
        num = num + shift - 26
      else
        num += shift
      end
...
class Hash
  def difference(other_hash)
    temp = self.merge(other_hash){|key,oldval,newval| [*oldval].to_a + [*newval].to_a }
    final = {}
    temp.each do |key, value|
      final[key] = value unless temp[key.to_sym].class == Array
    end
    return final
  end
end
...
class Fixnum
  def stringify(base)
    ...
    while (converter <= self)
      digit = (self / converter) % base
      if digit > 9 
        final += ref.key(digit)
      else  
        final += "#{(self / converter) % base}"
      end
